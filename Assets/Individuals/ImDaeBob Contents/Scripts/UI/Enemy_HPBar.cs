...
public class Enemy_HPBar : MonoBehaviour
{
    ...
    private void SetVisibility(bool isVisible)
    {
        ...
        if (_curTween != null && _curTween.IsActive())
            _curTween.Kill();
        if (isVisible)
            _curTween = gameObject.GetComponent<CanvasGroup>().DOFade(1f, 0.25f).SetEase(Ease.OutCirc);
        else
            _curTween = gameObject.GetComponent<CanvasGroup>().DOFade(0f, 0.25f).SetEase(Ease.InCirc);
    }
}
